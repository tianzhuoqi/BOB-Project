local ShipBodyMediator = class("ShipBodyMediator", MediatorDynamic)

function ShipBodyMediator:OnRegister()
    ...
    self:RegisterObserver(NotiConst.Notify_ShipBodyOpenTabItem, "OpenTabItem")
    self:RegisterObserver(NotiConst.Notify_ShipBodyEditHullProductLine, "EditHullProductLine")
    self:RegisterObserver(NotiConst.Notify_ShipBodyEditHullProductClick, "OnHullProductClick")
    self:RegisterObserver(NotiConst.Notify_ShipBodyGetHullFactData, "GetHullFactData")
    self:RegisterObserver(NotiConst.Notify_ShipBodyRefreshTime, "RefreshTime")
    self:RegisterObserver(NotiConst.Notify_ShipBodyRefreshView, "RefreshView")
end

function ShipBodyMediator:InitData()
    ...
    Facade:SendNotification('ShipBodyPanelProductLineNotify', self.maxLineCount)
    ...
    self.limitCount = DATA_BLDG_FUNC_HULL_FACT[index][DATA_BLDG_FUNC_HULL_FACT.EVar["temp_cap_n"]]
    local addition_limitCount = self.planetaryProxy:GetValueAfterAddition(self.limitCount,"BLDG_FUNC_HULL_FACT","temp_cap")
...
function ShipBodyMediator:RefreshView(notification)
    local modId = UnityEngine.PlayerPrefs.GetInt(self.planetaryProxy:GetFilterKey())
    self.modData = self.planetaryProxy:GetUserModById(modId)
    if self.modData == nil then
        self.m_viewComponent.uiBinder.m_Label_MbName.text = "模板过滤器"
    ...
    Facade:SendNotification(NotiConst.Notify_ShipBodyOpenTabItem, self.curTabIndex)
end

function ShipBodyMediator:OpenTabItem(notification)
...
function ShipBodyMediator:OpenTabItemNoRefresh()
    local hullType = self.curTabIndex + 1
    local shipHullData = self.planetaryProxy:GetShipHullConfigDataByType(hullType)

    local filterData = {}
    if self.modData == nil then
        filterData = shipHullData
    else
        for i,v in ipairs(shipHullData) do
            for j,k in ipairs(self.modData.hullParts) do
                if v.id == k.hullId then
    ...
    self.planetaryProxy:SetPackageShipHullData(filterData)
    Facade:SendNotification("ShipBodyPanelHullNotify", #filterData)
end

function ShipBodyMediator:Close()
...
function ShipBodyMediator:HullFilter()
    Facade:ReplacePanel("HullFilterPanel")
end

function ShipBodyMediator:ShowAssemble(items)
    local count = 0
    if items ~= nil then
...
--获取舰体厂数据
function ShipBodyMediator:GetHullFactData(notification)
...
function ShipBodyMediator:OnGetHullFactData(btsData)
    if btsData == nil then 
        LogDebug("ShipBodyMediator:OnGetHullFactData btsData==nil")
        self.planetaryProxy:SetHullfactMSGProductLine({})
...
--建筑队列修改
function ShipBodyMediator:EditHullProductLine(notification)
    ...
    if type == 0 then
        ...
        if (self.receiveCount + sum) >= self.limitCount then
            LogDebug("舰壳厂空间已经达到上限：{0}", self.limitCount)
            return
        end

        local shipHullData = self.planetaryProxy:GetPackageShipHullDataByIndex(index)
        for i = 1, #hullfactProductLineData do
            local tempData = hullfactProductLineData[i]
            if tempData.techId == shipHullData.data.id then
        ...
        if editData == nil then
            ...
            ProductLine.techId = shipHullData.data.id
...
function ShipBodyMediator:OnEditHullProductLine(btsData)
    LogDebug("ShipBodyMediator:OnEditHullProductLine")
    if btsData == nil then 
        LogDebug("ShipBodyMediator:OnEditHullProductLine btsData==nil")
        self.planetaryProxy:SetHullfactMSGProductLine({})
        self.planetaryProxy:SetHullfactMSGData({})
        self:ShowAssemble()
    else
...
function ShipBodyMediator:OnGetPeoductLineProduct(btsData)
    if btsData == nil then 
        LogDebug("ShipBodyMediator:OnGetPeoductLineProduct btsData==nil")
        return
    end
...
function ShipBodyMediator:OnHullProductClick(notify)
...
function ShipBodyMediator:JumHull()

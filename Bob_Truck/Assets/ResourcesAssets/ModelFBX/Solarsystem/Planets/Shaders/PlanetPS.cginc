...
inline PlanetData GetPlanetData(PlanetOutput p)
{
	...
	float2 mask2uv = pData.UV.xy+float2(_OceanMask2U,0);
	float2 mask2uvRg = tex2D(_OceanMask2,mask2uv).rg;
	pData.OceanMap += mask2uvRg;
	pData.OceanMap.r = (pData.OceanMap.r>0.5)?1:0;

	//pData.HeightMap = MixPoleMap(_HeightMap, pData.GradientMap, uvScaled, _HeightTile);
	pData.HeightMap = tex2D(_HeightMap, p.uv.xy);
...
{
	float3 mntColor = (1-p.HeightMap.b) * _MountainColor.rgb;
	float mask = (_MountainCoverage - (1-p.HeightMap.b)) * _MountainFactors;
	mask = saturate(lerp(0, 1, mask));

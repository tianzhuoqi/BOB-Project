...
inline PlanetData GetPlanetData(PlanetOutput p)
{
	...
	float4 surfaceNormal2uvScaled = pData.UV;
	surfaceNormal2uvScaled.x += _SurfaceNormalMap2U;
	surfaceNormal2uvScaled = float4(surfaceNormal2uvScaled.y , surfaceNormal2uvScaled.x, surfaceNormal2uvScaled.z * 0.5, surfaceNormal2uvScaled.w * 0.5);
	float3 normalMap2 = (UnpackPlanetNormal((MixPoleMap(_SurfaceNormalMap2, pData.GradientMap, surfaceNormal2uvScaled, _HeightTile)).xyz, _NormalHeight));
	pData.NormalMap = saturate(pow(pow(pData.NormalMap,4)+pow(normalMap2,4),0.25));

	pData.NormalMap = normalize(TangentToWorld(pData.NormalMap, p));
	
	pData.OceanMap = tex2D(_OceanMask, pData.UV.xy).rg;

	float2 mask2uv = pData.UV.xy+float2(_OceanMask2U,_OceanMask2V);
	float2 mask2uvRg = tex2D(_OceanMask2,mask2uv).rg;
	pData.OceanMap = saturate(pow(pow(mask2uvRg,4)+pow(pData.OceanMap,4),0.25));

	pData.HeightMap = MixPoleMap(_HeightMap, pData.GradientMap, uvScaled, _HeightTile);

	float4 height2uvScaled = pData.UV;
	height2uvScaled.x += _HeightMap2U;
	height2uvScaled = float4(height2uvScaled.y , height2uvScaled.x, height2uvScaled.z * 0.5, height2uvScaled.w * 0.5);
	float4 height2map = MixPoleMap(_HeightMap2, pData.GradientMap, height2uvScaled, _HeightTile);

	pData.HeightMap = saturate(pow(pow(pData.HeightMap,4)+pow(height2map,4),0.25));
	pData.HeightMap = tex2D(_HeightMap, pData.UV.xy);
...
inline float3 AddVegetationColor(float3 color, PlanetData p)
{
	...
	float mask = (_VegetationCoverage - p.HeightMap.b * p.HeightMap.r) * 1;
...
inline float3 AddWaterOptimise(float3 color, PlanetData p)
{
	...
	waterColor += (waterSpec ) * _WaterSpecularColor;
	return lerp(color, saturate(waterColor), p.OceanMap.r);	
}
...
inline float3 AddDiffuseOptimise(float3 color, PlanetData p, float backlightfactor)
{
	float diffuse = lerp(p.DiffuseFactorNM, p.DiffuseFactor,  p.OceanMap.r * 0.5);

	return color * (pow(diffuse, _DiffusePow) + backlightfactor);
}
...
inline float3 AddCloudsOptimise(float3 color, PlanetData p)
{
	float2 shadowUVBelly = p.LightDirTS.xy * 0.005;
	float4 cloudUV = p.UV;

	cloudUV.x += _CloudsAnimation * 0.15 * _Time;
	cloudUV.zw = RotateUV(cloudUV.zw, _Time, -_CloudsAnimation);


	float cloudBelly = tex2D(_CloudsMap, cloudUV.xy * float2(2, 1)).r;
	float cloudPole = tex2D(_CloudsMap, cloudUV.zw).r;
	float cloudColor = lerp(cloudPole, cloudBelly, p.GradientMap + 0.1);

	float cloudBellyShadow = tex2D(_CloudsMap, (cloudUV.xy + shadowUVBelly) * float2(2, 1)).r;

	cloudBellyShadow = saturate(pow(1 - cloudBellyShadow, _CloudsShadows));

	color *= cloudBellyShadow;
	color += max(p.shadow, 0.005) * saturate(cloudColor * _CloudsBrightness * _CloudsColor.rgb);
	return color;
}
...
inline float3 AddCityLightsOptimise(float3 color, PlanetData p)
{
	float dayNightFactor = max(p.LightDirTS.z, 0);
	float mask = tex2D(_CityLightMaskMap, p.UV.xy).x;
	float2 detailUV = tex2D( _CityLightUVMap, p.UV.xy ).xy;

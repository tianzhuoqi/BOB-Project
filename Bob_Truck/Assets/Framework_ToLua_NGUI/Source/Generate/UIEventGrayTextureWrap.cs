//this source code was auto-generated by tolua#, do not modify it
using System;
using LuaInterface;

public class UIEventGrayTextureWrap
{
	public static void Register(LuaState L)
	{
		L.BeginClass(typeof(UIEventGrayTexture), typeof(UITexture));
		L.RegFunction("SetGray", SetGray);
		L.RegFunction("SetVisible", SetVisible);
		L.RegFunction("SetEnabled", SetEnabled);
		L.RegFunction("SetNormal", SetNormal);
		L.RegFunction("__eq", op_Equality);
		L.RegFunction("__tostring", ToLua.op_ToString);
		L.EndClass();
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int SetGray(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 1);
			UIEventGrayTexture obj = (UIEventGrayTexture)ToLua.CheckObject<UIEventGrayTexture>(L, 1);
			obj.SetGray();
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int SetVisible(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 2);
			UIEventGrayTexture obj = (UIEventGrayTexture)ToLua.CheckObject<UIEventGrayTexture>(L, 1);
			bool arg0 = LuaDLL.luaL_checkboolean(L, 2);
			obj.SetVisible(arg0);
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int SetEnabled(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 2);
			UIEventGrayTexture obj = (UIEventGrayTexture)ToLua.CheckObject<UIEventGrayTexture>(L, 1);
			bool arg0 = LuaDLL.luaL_checkboolean(L, 2);
			obj.SetEnabled(arg0);
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int SetNormal(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 1);
			UIEventGrayTexture obj = (UIEventGrayTexture)ToLua.CheckObject<UIEventGrayTexture>(L, 1);
			obj.SetNormal();
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int op_Equality(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 2);
			UnityEngine.Object arg0 = (UnityEngine.Object)ToLua.ToObject(L, 1);
			UnityEngine.Object arg1 = (UnityEngine.Object)ToLua.ToObject(L, 2);
			bool o = arg0 == arg1;
			LuaDLL.lua_pushboolean(L, o);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}
}


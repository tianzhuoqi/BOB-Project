...
#region Create
	...
	static public void AddLabel ()
	{
		...
		if (go != null)
		{
            ...
            //多语言组件
            Selection.activeGameObject.AddComponent<NLanguageTextByKey>();
		}
		else Debug.Log("You must select a game object first.");
	}

    [MenuItem("NGUI/Create/Button", false, 6)]
    static public void AddButton()
    {
        ...
        if (go != null)
        {
            ...
            Selection.activeGameObject.name = "Button";
            UIButton uiBtn = Selection.activeGameObject.AddComponent<UIButton>();
            ...
            GameObject sprite = NGUISettings.AddSprite(Selection.activeGameObject).gameObject;
            uiBtn.tweenTarget = sprite.gameObject;
            UILabel label = NGUISettings.AddLabel(Selection.activeGameObject);
            //多语言组件
            label.gameObject.AddComponent<NLanguageTextByKey>();

            BoxCollider box = Selection.activeGameObject.AddComponent<BoxCollider>();
            UISprite uispr = sprite.GetComponent<UISprite>();
            box.size = new Vector3(uispr.width, uispr.height, 1);
        }
        else Debug.Log("You must select a game object first.");
    }

    [MenuItem("NGUI/Create/Editbox", false, 6)]
    static public void AddEditbox()
    {
        GameObject go = NGUIEditorTools.SelectedRoot(true);
        ...
        {
            Selection.activeGameObject = NGUISettings.AddLabel(go).gameObject;
            Selection.activeGameObject.name = "Editbox";
            BoxCollider box = Selection.activeGameObject.AddComponent<BoxCollider>();
            UIInput uiInput = Selection.activeGameObject.AddComponent<UIInput>();
            uiInput.label = Selection.activeGameObject.GetComponent<UILabel>();
            box.size = new Vector3(uiInput.label.width, uiInput.label.height, 1);
        }
        else Debug.Log("You must select a game object first.");
    }

...
/// 所有资源Loader继承这个
/// </summary>
public class NAbstractResourceLoader : IAsyncObject
{
    public delegate void LoaderDelgate(bool isOk, object resultObject);
    private static readonly Dictionary<Type, Dictionary<string, NAbstractResourceLoader>> _loadersPool =
           new Dictionary<Type, Dictionary<string, NAbstractResourceLoader>>();
    ...
    /// </summary>
    private static readonly List<NAbstractResourceLoader> CacheLoaderToRemoveFromUnUsed =
        new List<NAbstractResourceLoader>();

    /// <summary>
    /// 进行垃圾回收
    /// </summary>
    internal static readonly Dictionary<NAbstractResourceLoader, float> UnUsesLoaders =
        new Dictionary<NAbstractResourceLoader, float>();
    ...
    protected static Dictionary<string, NAbstractResourceLoader> GetTypeDict(Type type)
    {
        Dictionary<string, NAbstractResourceLoader> typesDict;
        if (!_loadersPool.TryGetValue(type, out typesDict))
        {
            typesDict = _loadersPool[type] = new Dictionary<string, NAbstractResourceLoader>();
        }
        return typesDict;
    }
    ...
    public static int GetRefCount<T>(string url)
    {
        ...
        NAbstractResourceLoader loader;
    ...
    protected static T AutoNew<T>(string url, LoaderDelgate callback = null, bool forceCreateNew = false,
        params object[] initArgs) where T : NAbstractResourceLoader, new()
    {
        Dictionary<string, NAbstractResourceLoader> typesDict = GetTypeDict(typeof(T));
        NAbstractResourceLoader loader;
        if (string.IsNullOrEmpty(url))
        {
            Debug.LogError("[" + typeof(T).ToString() + ":AutoNew]url为空");
        }
    ...
    protected NAbstractResourceLoader()
    {
        RefCount = 0;
    }
        ...
        if (callback != null)
        {
            if (IsCompleted)
            {
                if (ResultObject == null)
                    Debug.LogWarning("Null ResultAsset " + Url);
    ...
    public virtual void Release()
    {
        ...
        {
            Debug.LogWarning("[" + GetType().Name + "]Too many dipose! " + this.Url + ", Count: " + RefCount);
        }
        ...
        if (RefCount <= 0)
        {
            ...
            if (Debug.isDebugBuild)
            {
                if (RefCount < 0)
                {
                    Debug.LogError("[" + GetType() + "]RefCount< 0, " + GetType().Name + " : " + Url + ", NowRefCount: " + RefCount + ", Will be fix to 0");
                ...
                {
                    Debug.LogError("[" + GetType() + "]UnUsesLoader exists: " + this.ToString());
                }
            }
    ...
    private void Dispose()
    {
        ...
        if (!IsForceNew)
        {
            ...
            //if (Url != null) // TODO: 以后去掉
            {
                ...
                if (!bRemove)
                {
                    Debug.LogWarning("[" + type.Name + ":Dispose]No Url: " + Url + ", Cur RefCount: " + RefCount);
                }
            }
        }

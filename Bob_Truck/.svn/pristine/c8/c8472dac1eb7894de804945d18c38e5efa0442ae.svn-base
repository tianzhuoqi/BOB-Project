...
inline float2 RotateUVAtCenter(float2 uv,float angle)
{
	float2 cUV = uv - float2(0.5,0.5);
	float d = radians(angle % 360);
	cUV.x = (cos(d) * cUV.x + sin(d) * cUV.y)/1.414 + 0.5;
	cUV.y = (cos(d) * cUV.y + sin(d) * cUV.x)/1.414 + 0.5;
	return cUV;
}
